...
void on_write(uv_write_t* _req, int _status)
{
    ...
    // 開始讀取數據
    uv_read_start(_req->handle, on_alloc, on_read);
}

void on_connect(uv_connect_t* _server, int _status)
{
    SPDLOG_INFO("status: {}", _status);
    if (_status < 0)
    {
    ...
    *message += '\0';
    ...
    uv_write(req, _server->handle, &wrbuf, 1, on_write);
}
...
int main()
{
    ...
    socket->data = (void*)(new std::vector<char>());
